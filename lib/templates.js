...
Templates.build = function(method, params, data) {
    ...
    response += "xmlns='AnetApi/xml/v1/schema/AnetApiSchema.xsd'>";
...
Templates.getSettledBatchList = function(data) {
    var response = '';
    var err = false;
    var errors = [];

    response += (data.includeStatistics)
        ? "<includeStatistics>" + data.includeStatistics + "</includeStatistics>"
        : '';

    response += (data.firstSettlementDate)
        ? "<firstSettlementDate>" + data.firstSettlementDate + "</firstSettlementDate>"
        : '';


    response += (data.lastSettlementDate)
        ? "<lastSettlementDate>" + data.lastSettlementDate +"</lastSettlementDate>"
        : '';

    return (err) ? errors : response;
};

Templates.getTransactionList = function(data) {
    var response = '';
    var err = false;
    var errors = [];
...
Templates.getTransactionDetails = function(data) {
    var response = '';
    var err = false;
    var errors = [];
...
Templates.createTransaction = function(data) {
    var response = "", err = false, insert;
    var errors = [];
    ...
    response += "<transactionType>"+data.transactionType+"</transactionType>";
    ...
    if ("payment" in data) {
        response += "<payment>";
        response += jstoxml.toXML(data.payment);
        response += "</payment>";
    }

    if ("order" in data) {
        response += "<order>";
        response += jstoxml.toXML(data.order);
        response += "</order>";
    }

    if ("customer" in data) {
        response += "<customer>";
        response += jstoxml.toXML(data.customer);
        response += "</customer>";
    }

    if ("billTo" in data) {
        response += "<billTo>";
        response += jstoxml.toXML(data.billTo);
        response += "</billTo>";
    }

    if ("shipTo" in data) {
        response += "<shipTo>";
        response += jstoxml.toXML(data.shipTo);
        response += "</shipTo>";
    }

    if ("transactionSettings" in data) {
        response += "<transactionSettings>";
        response += jstoxml.toXML(data.transactionSettings);
        response += "</transactionSettings>";
    }
